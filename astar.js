...
var avoidastar = {
  ...
  search: function(grid, start, end, p, temp) {
    astar.init(grid);
    var heuristic = avoidastar.heur;
    ...
    while (openHeap.size() > 0) {
      ...
      for (var i = 0, il = neighbors.length; i < il; i++) {
        ...
        if (!beenVisited || gScore < neighbor.g) {
          ...
          neighbor.visited = true;
          neighbor.parent = currentNode;
          neighbor.h = neighbor.h || heuristic(p, temp);
          neighbor.g = gScore;
          neighbor.f = neighbor.g + neighbor.h;
          neighbor.debug =
            "F: " +
  ...
  heur: function(p, temp) {
    return getManhattan(p.nearEnemy[0].p.x, p.nearEnemy[0].p.y, p.x, p.y);
  }, // manhattan
    ...
    while (openHeap.size() > 0) {
      ...
      for (var i = 0, il = neighbors.length; i < il; i++) {
        ...
        if (!beenVisited || gScore < neighbor.g) {
          ...
          neighbor.h = neighbor.h || heuristic(neighbor.pos, end.pos);
          neighbor.g = gScore;
          neighbor.l = qred[neighbor.x][neighbor.y];
          neighbor.f =
            neighbor.h +
            neighbor.g +
            10 * qred[neighbor.x][neighbor.y] * Math.random() * 1100;
          neighbor.debug =
            "F: " +
            neighbor.f +
            "<br />G: " +
    ...
    while (openHeap.size() > 0) {
      ...
      for (var i = 0, il = neighbors.length; i < il; i++) {
        ...
        if (!beenVisited || gScore < neighbor.g) {
          ...
          // ctx.drawImage(
          //   red,
          //   (MARGIN + WIDTH) * neighbor.x + MARGIN,
          //   (MARGIN + WIDTH) * neighbor.y + MARGIN
          // );

          // var text =
          //   heuristic(rr, neighbor) + heuristic(neighbor, { x: 30, y: 5 });

          // ctx.fillText(
          //   text,
          //   (MARGIN + WIDTH) * neighbor.x + MARGIN,
          //   (MARGIN + WIDTH) * neighbor.y + 30 + MARGIN
          // );
          // ctx.globalAlpha = 1;
